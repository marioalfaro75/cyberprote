    ...
    <div>
      ...
      {findings.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4 mb-4">
          <p className="text-xs text-gray-500 mb-1">Severity Distribution</p>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <button
          ...
          className={`px-3 py-1 rounded text-sm font-medium ${filter === null ? 'bg-gray-800 text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
        >
          All ({findings.length})
        </button>
        {[5, 4, 3, 2, 1].map((id) => (
                  ...
                  <td colSpan={4} className="px-4 py-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          ...
                          <ul className="space-y-1">
                            {props.resources.map((r, ri) => (
                              <li key={ri} className="text-gray-700">
                                ...
                                {r.region && <span className="text-gray-400 ml-1">— {r.region}</span>}
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      {props.vulnerabilities && props.vulnerabilities.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Vulnerabilities</p>
                          <ul className="space-y-1">
                            {props.vulnerabilities.map((v, vi) => (
                              <li key={vi} className="text-gray-700">
                                {v.cve?.uid && <span className="font-mono font-medium">{v.cve.uid}</span>}
                                {v.cvss?.[0]?.base_score != null && (
                                  <span className="ml-2 text-gray-500">CVSS: {v.cvss[0].base_score}</span>
                                )}
                              </li>
                            ))}
                          </ul>
                      ...
                      {props.remediation?.description && (
                        <div>
                          ...
                          <p className="text-gray-700">{props.remediation.description}</p>
                        </div>
                      )}
                      ...
                      {props.cloud && (
                        <div>
                          <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Cloud Context</p>
